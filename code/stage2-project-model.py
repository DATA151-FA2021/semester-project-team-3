...
labels = np.unique(y_val)
cm = confusion_matrix(y_val, y_pred > 0.2, labels=labels)
pd.DataFrame(cm, index=labels, columns=labels) #lower the threshold, increase the sensitivity 20% Chance of fail
...
print('Accuracy score is:', accuracy_score(y_val, y_pred >.2))
print('Recall score is:', recall_score(y_val, y_pred >.2))
print('Precision score is:', precision_score(y_val, y_pred >.2))
